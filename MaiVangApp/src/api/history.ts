...
export async function sendHistoryImage(id: number, image: SelectedImage, question?: string): Promise<InteractionDto> {
  ...
  try {
    ...
    if (error instanceof ApiError && (error.status === 502 || error.status === 503)) throw new ApiError('Dịch vụ AI đang khởi động hoặc tạm thời gián đoạn. Vui lòng thử lại.', error.status, error.fieldErrors, error.kind, error.debugDetail);
