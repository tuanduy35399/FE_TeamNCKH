...
export function DiagnosisScreen() {
  ...
  async function chooseLibrary() {
    ...
    try {
      ...
      if (!permission.granted) { setPickerError('Bạn chưa cấp quyền truy cập thư viện ảnh.'); return; }
  ...
  async function submit() {
    const cleanText = text.trim(); if (!image && !cleanText) return;
    ...
    try { setResult(await submitDiagnosis({ image, text: cleanText || undefined })); }
  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}><KeyboardAvoidingView style={styles.fill} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    <ScrollView ref={scrollRef} testID="diagnosis-screen" contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      {!result ? <>
        <Text style={styles.intro}>Thêm ảnh lá mai và mô tả dấu hiệu bạn quan sát được.</Text>
        ...
        {hasText && !image ? <View style={styles.info}><Ionicons name="information-circle-outline" size={18} color={colors.primary} /><Text style={styles.infoText}>Bạn đang hỏi thông tin chung. Thêm ảnh nếu muốn kiểm tra tình trạng lá cụ thể.</Text></View> : <Text style={styles.helper}>Bạn có thể chỉ gửi ảnh. Thêm mô tả sẽ giúp kết quả phù hợp hơn.</Text>}
      ...
      </> : submittedMode === 'text' ? <AssistantConversation question={submittedText} answer={result.answer || 'Chưa có nội dung trả lời.'} onNew={reset} /> : <DiagnosisResult result={result} originalText={submittedText} onNewDiagnosis={reset} />}
    </ScrollView>
    <ImageSourceSheet visible={sheetOpen} error={pickerError} onCamera={openCamera} onLibrary={() => void chooseLibrary()} onClose={() => { setSheetOpen(false); setPickerError(''); }} />
...
const styles = StyleSheet.create({
  ...
  content: { padding: spacing.md, paddingBottom: 28, gap: 14 }, intro: { color: colors.muted, lineHeight: 21 }, upload: { minHeight: 176, padding: spacing.md, alignItems: 'center', justifyContent: 'center', borderRadius: radius.lg, borderWidth: 1.5, borderStyle: 'dashed', borderColor: '#8EB59C', backgroundColor: colors.surface }, uploadIcon: { width: 54, height: 54, borderRadius: 27, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center', marginBottom: 9 }, uploadTitle: { color: colors.text, fontSize: 18, fontWeight: '900' }, uploadText: { color: colors.muted, textAlign: 'center', marginTop: 4 }, uploadAction: { minHeight: 34, flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 7 }, uploadActionText: { color: colors.primary, fontWeight: '800' }, pressed: { opacity: .76 },
  ...
  field: { gap: 7 }, label: { color: colors.text, fontWeight: '800', fontSize: 14 }, textarea: { minHeight: 104, maxHeight: 170, padding: 13, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, color: colors.text, fontSize: 15, lineHeight: 21 }, helper: { color: colors.muted, fontSize: 13, lineHeight: 19 }, info: { flexDirection: 'row', gap: 8, padding: 11, borderRadius: radius.md, backgroundColor: colors.primarySoft }, infoText: { flex: 1, color: colors.primaryDark, fontSize: 13, lineHeight: 18 }, error: { flexDirection: 'row', gap: 8, padding: 12, borderRadius: radius.md, backgroundColor: colors.dangerSoft }, errorText: { flex: 1, color: colors.danger, lineHeight: 20 }, submit: { minHeight: 52, borderRadius: radius.md, backgroundColor: colors.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, submitDisabled: { backgroundColor: colors.disabled }, submitText: { color: '#fff', fontWeight: '900', fontSize: 16 },
});
