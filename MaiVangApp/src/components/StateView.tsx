...
export function StateView({ icon = 'leaf-outline', title, message, actionLabel, onAction }: { icon?: keyof typeof Ionicons.glyphMap; title: string; message: string; actionLabel?: string; onAction?: () => void }) {
  return <View style={styles.wrap}><Ionicons name={icon} size={42} color={colors.primary} /><Text style={styles.title}>{title}</Text><Text style={styles.body}>{message}</Text>{actionLabel && onAction && <View style={styles.button}><AppButton title={actionLabel} onPress={onAction} variant="secondary" /></View>}</View>;
}
